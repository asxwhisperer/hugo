...
elements.forEach((el) => {
  ...
  for (const match of matches) {
    const tickerEl = document.createElement(
      el.classList.contains("title-link") ? "span" : "a"
    );
    ...
    if (!el.classList.contains("title-link")) {
      // If it's an "a" tag, set the href attribute and onclick attribute
      tickerEl.href = url;
